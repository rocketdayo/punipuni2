...
// Preload S-rank individual character images
// Each S-rank char has an id like "char_s_1" ... "char_s_10"
// Images are named /puni_s_1.png ... /puni_s_10.png (fallback: /puni_s.png)
const S_CHAR_IMAGES: Record<string, HTMLImageElement> = {};
const loadSImage = (charId: string, src: string) => {
  const img = new Image();
  img.src = src;
  S_CHAR_IMAGES[charId] = img;
};
// Pre-load the generic S image as fallback for all S chars
(['char_s_1','char_s_2','char_s_3','char_s_4','char_s_5',
  'char_s_6','char_s_7','char_s_8','char_s_9','char_s_10'] as const).forEach((id, i) => {
  loadSImage(id, `/puni_s.png`); // same image for now; replace with /puni_s_${i+1}.png when individual arts are ready
});
  ...
  useEffect(() => {
    ...
    const buildScene = (W: number, H: number) => {
      ...
      Events.on(render, 'afterRender', () => {
        ...
        ctx.stroke();
        // Draw puni sprites (S-rank = circular image, others = emoji)
        ...
        for (const b of Composite.allBodies(engine.world).filter(b => !b.isStatic)) {
          ...
          if (cd.rank === 'S') {
            // Draw circular-clipped image for S-rank
            const img = S_CHAR_IMAGES[cd.id];
